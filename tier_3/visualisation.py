...
from judge.grid_race_env import CellType
...
class Screen:
    ...
    class TrackColours(NamedTuple):
        ...
        goal_cells: tuple[pygame.Color, pygame.Color] = (pygame.Color('black'),
                                                         pygame.Color('white'))
        fog: pygame.Color = pygame.Color(50, 50, 50, 180)
        oil: pygame.Color = pygame.Color(0, 0, 0)
        sand: pygame.Color = pygame.Color(255, 255, 0)
        ...
        for i, row in enumerate(env_info.track):  # row_ind
            for j, cell in enumerate(row):  # col_ind
                ...
                pygame.draw.line(
                    ...
                    width=border_width)
                # draw goal field
                if c[i][j] > CellType.START.value:
                    # draw special cells
                    match c[i][j]:
                        case CellType.GOAL.value:
                            colour = self.TRACK_COLOURS.goal_cells[(i+j) % 2]
                        case CellType.OIL.value:
                            colour = self.TRACK_COLOURS.oil
                        case CellType.SAND.value:
                            colour = self.TRACK_COLOURS.sand
                        case _:
                            print(f'Warning: unknown cell type: {c[i][j]}')
                            colour = self.TRACK_COLOURS.road
                    pygame.draw.rect(
                        grid_surface, colour,
        ...
        for row_ind in range(height):
            for col_ind in range(width):
                if ((player.x - row_ind)**2 +
                    ...
                    pygame.draw.rect(
                        surface, self.TRACK_COLOURS.fog,
    ...
    def print_info(self, t: int | str, last_step: Optional[replay.PlayerStep]):
        line = self.font.render(f'Turn: {t}', True, self.FONT_COLOUR,
        ...
        if last_step is not None:
            ...
            player_legend = self.font.render(
                ...
                self.PLAYER_COLOURS[last_step.player_ind],
                self.BACKGROUND_COLOUR)
